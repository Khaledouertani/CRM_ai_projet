...
from services.rgpd import sanitize_for_rgpd
from ui.style import inject_custom_css, detail_card, COLORS
...
with tab1:
    ...
    if "pending_analysis" in st.session_state:
        ...
        if labeled:
            ...
            # Affiche les lignes avec couleur par rôle (SaaS style)
            for line in labeled.split("\n"):
                if line.startswith("[Agent]"):
                    st.markdown(
                        f"<div style='background:rgba(16, 185, 129, 0.1);border-left:4px solid {COLORS['success']};"
                        f"padding:12px 16px;border-radius:12px;margin:8px 0;font-size:14px;color:var(--text-primary);'>"
                        f"<b style='color:{COLORS['success']}'>Agent:</b> {line.replace('[Agent]', '').strip()}</div>", unsafe_allow_html=True
                    )
                elif line.startswith("[Client]"):
                    st.markdown(
                        f"<div style='background:rgba(59, 130, 246, 0.1);border-left:4px solid {COLORS['info']};"
                        f"padding:12px 16px;border-radius:12px;margin:8px 0;font-size:14px;color:var(--text-primary);'>"
                        f"<b style='color:{COLORS['info']}'>Client:</b> {line.replace('[Client]', '').strip()}</div>", unsafe_allow_html=True
                    )
        ...
        if keywords:
            ...
            tags_html = "".join([
                f"<span style='background:{COLORS['accent_bg']};color:{COLORS['accent_hover']};border:1px solid {COLORS['accent']};"
                f"border-radius:20px;padding:4px 12px;margin:3px;font-size:13px;'>{k}</span>"
                for k in keywords
            ])
        ...
        # Enregistrement avec conformité RGPD
        if st.button(" Enregistrer dans la Base de Données", type="secondary"):
            call_data = {
                ...
                "qualification_detected": result.get("qualification_detected", ""),
                "qualification_match":    result.get("qualification_match", True),
                "coherence_score":       result.get("coherence_score", 100),
            ...
            # Appliquer l'anonymisation RGPD avant insertion
            sanitized_data = sanitize_for_rgpd(call_data)
            call_id = insert_call(sanitized_data)
            st.success(" Appel enregistré dans MySQL (anonymisé RGPD).")

            # Créer un follow-up si RDV détecté
            try:
                from services.followup import create_followup_from_call
                if result.get("appointment_confidence", 0) >= 30 and result.get("appointment_date"):
                    followup_id = create_followup_from_call(result, call_id)
                    if followup_id:
                        st.toast(f"Follow-up créé (ID: {followup_id})")
            except Exception as e:
