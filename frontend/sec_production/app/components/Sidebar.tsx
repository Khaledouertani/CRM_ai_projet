...
import { 
  ...
  Plug, MessageCircle, Sparkles, Shield, Settings, Sliders, Bell, Activity, Menu
} from 'lucide-react';
...
const adminNavSections: NavSection[] = [
  ...
  },
  {
    label: 'Performance', icon: Activity, children: [
      { label: 'Classement', icon: Activity, path: '/admin/dashboard' },
...
export function Sidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void; }) {
  ...
  const { user } = useAuth();
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(['Dashboard', 'Configuration']));
      ...
      <div className="p-4 border-b border-sidebar-border flex items-center justify-between">
        {!collapsed && (
          <div className="flex flex-col">
            <span className="font-black text-xl italic tracking-tighter text-primary">AI CRM</span>
            <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground opacity-60">Production v2.0</span>
          </div>
        )}
        <button onClick={onToggle} className="p-2 rounded-md hover:bg-sidebar-accent transition-colors">
                ...
                <div key={section.label} className="space-y-1">
                  <button
                    ...
                    className={cn(
                      ...
                      hasActiveChild ? "bg-sidebar-accent text-sidebar-accent-foreground" : "hover:bg-sidebar-accent"
                    )}
                  >
              ...
              <div key={section.label} className="group relative">
                <Link
                  ...
                  className={cn(
                    ...
                    section.children.some(c => isActive(c.path)) ? "bg-sidebar-accent" : "hover:bg-sidebar-accent"
                  )}
                >
                  <section.icon className="h-5 w-5" />
                </Link>
                <div className="absolute left-full top-0 ml-2 hidden group-hover:block z-50 min-w-48 bg-background border rounded-md shadow-lg p-2 space-y-1">
